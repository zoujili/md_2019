...
func SetupTestOptions() []Option {
	if config.GetConfig().IsMysqlAssignWorkerID() {
		return MySQLAssignerOptions
	} else {
		return ManualAssignOptions
	}
...
func TestNewNode(t *testing.T) {
	...
	_, err := NewNode(0, SetupTestOptions()...)
	if err != nil {
		t.Fatalf("error creating NewNode, %s", err)
	}

	_, err = NewNode(50000000, SetupTestOptions()...)
	if err == nil {
...
func TestGenerateDuplicateID(t *testing.T) {
	...
	node, _ := NewNode(1, SetupTestOptions()...)
...
func TestPrintAll(t *testing.T) {
	node, err := NewNode(0, SetupTestOptions()...)
	if err != nil {
		t.Fatalf("error creating NewNode, %s", err)
	}
